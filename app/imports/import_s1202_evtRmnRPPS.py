...
from models.esocial_s1202_evtRmnRPPS import ESocialS1202EvtRmnRPPS
import json
import logging
...
def import_s1202_evtRmnRPPS(
    xml_content: str | bytes,
    source_file: str = None,
    municipio_cnpj: str = None,
) -> dict:

    xml_data = esocial_data(tipo_xml='evtRmnRPPS', xml_content=xml_content)
    ...
    if ESocialS1202EvtRmnRPPS.query.filter_by(evtRmnRPPSId=evtRmnRPPSId).first():
        from models.import_skip import ImportSkip
        details = {"evtRmnRPPSId": evtRmnRPPSId, "nrInsc": nrInsc, "perApur": perApur, "arquivo_origem": source_file}
        skip = ImportSkip(event_type='S-1202 evtRmnRPPS', nrInsc=nrInsc, cpf=None, perApur=perApur or None, arquivo_origem=source_file, details=str(details))
        db.session.add(skip)
        db.session.commit()
        return {"sucess": False, "error": "evtRmnRPPSId already exists", "details": [details]}

    dmDev = evt.find('.//{*}dmDev')
    ideEstab = dmDev.find('.//{*}infoPerApur//{*}ideEstab') if dmDev is not None else None
    remunPerApur = ideEstab.find('.//{*}remunPerApur') if ideEstab is not None else None
    itensRemun_parts = []
    ...
    rec = ESocialS1202EvtRmnRPPS(
        evtRmnRPPSId = evtRmnRPPSId or None,
        substituido = False,
